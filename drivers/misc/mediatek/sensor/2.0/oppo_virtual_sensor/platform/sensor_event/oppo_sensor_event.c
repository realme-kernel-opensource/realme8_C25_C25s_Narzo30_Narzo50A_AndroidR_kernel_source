...
struct oppo_sensor_event_client {
    spinlock_t buffer_lock;
    unsigned int head;
    unsigned int tail;
    unsigned int bufsize;
    unsigned int buffull;
    struct oppo_sensor_event *buffer;
    wait_queue_head_t wait;
};
struct oppo_sensor_event_obj {
    struct oppo_sensor_event_client client[MINOR_NUM_MAX + 1];
};
static struct oppo_sensor_event_obj *event_obj;
static struct lock_class_key buffer_lock_key[MINOR_NUM_MAX + 1];
/*
 * sensor_input_event only support process context.
 */
int oppo_sensor_input_event(unsigned char handle, const struct oppo_sensor_event *event)
{
    struct oppo_sensor_event_client *client = &event_obj->client[handle];
...
static int oppo_sensor_event_fetch_next(struct oppo_sensor_event_client *client,
              ...
              char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if (count != 0 && count < sizeof(struct oppo_sensor_event)) {
    ...
    for (;;) {
        ...
        while (read + sizeof(struct oppo_sensor_event) <= count &&
               oppo_sensor_event_fetch_next(client, &event)) {

            if (copy_to_user(buffer + read, &event,
                     sizeof(struct oppo_sensor_event)))
                return -EFAULT;

            read += sizeof(struct oppo_sensor_event);
        }
...
{
    struct  oppo_sensor_event_obj *obj = event_obj;
...
static int __init oppo_sensor_event_entry(void)
{
    struct oppo_sensor_event_obj *obj =
        kzalloc(sizeof(struct oppo_sensor_event_obj), GFP_KERNEL);

    event_obj = obj;
    return 0;
}
subsys_initcall(oppo_sensor_event_entry);
