...
static int oplus_get_usbtemp_volt(void)
{
	...
	pr_info("usbtemp_volt: %d\n", usbtemp_volt);

	return usbtemp_volt;
}

/*Shouli.Wang@ODM_WT.BSP.CHG 2021/03/25, add for usbtemp*/
...
/*Shouli.Wang@ODM_WT.BSP.CHG 2021/3/25, add for usbtemp*/
...
int get_chgntc_adc_temp(struct charger_device *chg_dev, int *tmin, int *tmax)
{
	...
	gpio_set_value(pinfo->data.chgntc_pin, CHGNTC_ENABLE_LEVEL);
	mdelay(500);
	usbtemp_volt = oplus_get_usbtemp_volt();
	*tmin = usb_port_volt_to_temp(usbtemp_volt);
	usbtemp_volt = oplus_get_usbtemp_volt();
	*tmax = usb_port_volt_to_temp(usbtemp_volt);
	gpio_set_value(pinfo->data.chgntc_pin, CHGNTC_DISABLE_LEVEL);
...
static bool oplus_chg_get_vbus_status(struct oplus_chg_chip *chip)
{
	...
	charger_type = chip->chg_ops->get_charger_volt();
	if(charger_type)
		return true;
	else
		return false;
